...
class GaussianRenderer(nn.Module):
    ...
    def compute_projection(
        ...
        J_proj[:, :, :2] = K.T  # (N, 2, 3)
        J_proj[:, :, 2] = -K.T @ cam_points[:, :2] / depths.unsqueeze(-1)  # (N, 2, 3)
        
        # Transform covariance to camera space
        covs_cam = covs3d @ R.T @ R  # (N, 3, 3)
    ...
    def compute_gaussian_values(
        ...
        gaussian = torch.exp(-0.5 * torch.sum(dx.unsqueeze(-1) @ inv_covs2D @ dx, dim=-2)) / (2 * np.pi * torch.sqrt(det))
        return gaussian
    ...
    def forward(
        ...
        # 7. Compute weights
        betas = 1-alphas
        betascum = betas.cumprod(dim=0)  # (N, H, W)
        betascum = betascum / betas  # (N, H, W)
        weights = alphas * betascum # (N, H, W)
