        ...
        if show:
            with imgui_utils.grayed_out(self.disabled_time != 0):
                ...
                if imgui_utils.button('Smile', width=viz.button_w, enabled='image' in viz.result):                  
                    x1 = int(self.feature_points[48][0])
                    y1 = int(self.feature_points[48][1])
                    x2 = int(self.feature_points[54][0])
                    y2 = int(self.feature_points[54][1])
                    self.points.append([y1,x1])
                    self.points.append([y2,x2])
                    self.targets.append([y1-30,x1-30])
                    self.targets.append([y2-30,x2+30])

                imgui.text(' ')
                imgui.same_line(viz.label_w)
                if imgui_utils.button('closeeyes', width=viz.button_w, enabled='image' in viz.result):                  
                    x1 = int(self.feature_points[37][0])
                    y1 = int(self.feature_points[37][1])
                    self.points.append([y1,x1])
                    self.targets.append([y1+10,x1])
                    x2 = int(self.feature_points[38][0])
                    y2 = int(self.feature_points[38][1])
                    self.points.append([y2,x2])
                    self.targets.append([y2+10,x2])
                    x3 = int(self.feature_points[40][0])
                    y3 = int(self.feature_points[40][1])
                    self.points.append([y3,x3])
                    self.targets.append([y3-10,x2])
                    x4 = int(self.feature_points[41][0])
                    y4 = int(self.feature_points[41][1])
                    self.points.append([y4,x4])
                    self.targets.append([y4-10,x4])
                    x1 = int(self.feature_points[43][0])
                    y1 = int(self.feature_points[43][1])
                    self.points.append([y1,x1])
                    self.targets.append([y1+10,x1])
                    x2 = int(self.feature_points[44][0])
                    y2 = int(self.feature_points[44][1])
                    self.points.append([y2,x2])
                    self.targets.append([y2+10,x2])
                    x3 = int(self.feature_points[46][0])
                    y3 = int(self.feature_points[46][1])
                    self.points.append([y3,x3])
                    self.targets.append([y3-10,x2])
                    x4 = int(self.feature_points[47][0])
                    y4 = int(self.feature_points[47][1])
                    self.points.append([y4,x4])
                    self.targets.append([y4-10,x4])

                imgui.text(' ')
                imgui.same_line(viz.label_w)
                if imgui_utils.button('facethinning', width=viz.button_w, enabled='image' in viz.result):
                    for i in range(1, 7):                                       
                        x1 = int(self.feature_points[i-1][0])
                        y1 = int(self.feature_points[i-1][1])
                        self.points.append([y1,x1])
                        self.targets.append([y1,x1+30])
                    for i in range(11, 17):                                       
                        x1 = int(self.feature_points[i][0])
                        y1 = int(self.feature_points[i][1])
                        self.points.append([y1,x1])
                        self.targets.append([y1,x1-30])
