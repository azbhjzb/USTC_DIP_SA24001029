...
def create_mask_from_points(points, img_h, img_w):
    ...
    for i in range(0, points.shape[0]):
        ...
        if p1[1] == p2[1]:
            continue
        px = p1[0] + (p2[0] - p1[0]) / (p2[1] - p1[1]) * (Y - p1[1])
        pup = p1[1]
        if p2[1] > p1[1]:
            pup = p2[1]
        mask = mask + (px > X) * (((Y - p1[1]) * (Y - p2[1]) < 0) + (Y == pup))
    mask = 255 * (mask % 2)
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask, dy, dx, device):
    """
    ...
    foreground_laplacian = torch.nn.functional.conv2d(foreground_img, laplacian_kernel, padding=1, groups=3).to(device)
    blended_img_laplacian = torch.nn.functional.conv2d(blended_img, laplacian_kernel, padding=1, groups=3).to(device)
    x_low = 0
    if dx < 0:
        x_low = -dx
    y_low = 0
    if dy < 0:
        y_low = -dy
    x_high = foreground_img.shape[3]
    if x_high > blended_img.shape[3] - dx:
        x_high = blended_img.shape[3] - dx
    y_high = foreground_img.shape[2]
    if y_high > blended_img.shape[2] - dy:
        y_high = blended_img.shape[2] - dy
    X, Y = np.meshgrid(np.arange(x_low, x_high), np.arange(y_low, y_high))
    loss = torch.sum(((blended_img_laplacian[:, :, Y + dy, X + dx] - foreground_laplacian[:, :, Y, X])
                     * foreground_mask[:, :, Y, X]) ** 2).to(device)
...
def blending(foreground_image_original, background_image_original, dx, dy, polygon_state):
    ...
    for step in range(iter_num):
        ...
        loss = cal_laplacian_loss(fg_img_tensor, fg_mask_tensor, blended_img_for_loss, bg_mask_tensor, dy, dx, device)
